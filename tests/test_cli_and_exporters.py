"""Unit tests for CLI commands and JSON/Markdown/SARIF/HTML/LLM exporters."""
...
def _dummy_report() -> DetectionReport:
    ev = Evidence(
        rule_code="LUA_TEST",
        description="Test heuristic for Lua metatables",
        weight=0.95,
        location=SourceLocation("game.lua", 10),
    )
    det = Detection(
        ...
        primary_location=SourceLocation("game.lua", 10),
        evidences=[ev],
    )
...
def test_cli_rules_command() -> None:
    ...
    assert "DPX-Lua" in result.stdout
    assert "Metatable" in result.stdout


def test_cli_info_command() -> None:
    result = runner.invoke(app, ["info", "metatable_prototype_oop"])
    assert result.exit_code == 0
    assert "Metatable Prototype OOP" in result.stdout
...
def test_exporters_format() -> None:
    ...
    assert "# 🌙 DPX-Lua: Lua & Luau Architectural Pattern Report" in md_out
    ...
    assert "DPX-Lua Architecture & GameDev Observability HUD" in html_out

    llm_out = LlmReportFormatter().format_scan_report(rep)
    assert '<codebase_architecture_analysis language="lua">' in llm_out
