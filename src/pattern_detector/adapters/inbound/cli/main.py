"""Typer and Rich CLI interface for DPX-Lua."""
...
app = typer.Typer(
    name="dpx-lua",
    help="🌙 Enterprise Lua & Luau Static Analyzer: Metatable OOP, Coroutines, LuaJIT FFI, GameDev Architectures (Roblox/Neovim/Love2D), GoF 23 & Global Leak Hazards.",
...
def scan(
    path: Annotated[
        str,
        typer.Argument(
            help="Path to a Lua/Luau source file (.lua, .luau) or directory containing a Lua project.",
        ),
    ] = ".",
    ...
    pattern: Annotated[
        ...
        typer.Option(
            "--pattern",
            "-p",
            help="Filter for specific pattern type(s) (e.g. 'metatable_prototype_oop', 'coroutine_cooperative_task').",
        ),
    ] = None,
    ...
    exclude: Annotated[
        ...
        typer.Option(
            "--exclude",
            "-e",
            help="Directory name(s) to exclude from scanning (e.g. -e lua_modules -e .luarocks).",
        ),
    ] = None,
    verbose: Annotated[
        bool,
    ...
    """Scan a Lua / Luau codebase for metatables, coroutine schedulers, LuaJIT FFI, GameDev signals, GoF patterns, and memory leaks."""
...
@app.command()
def rules() -> None:
    """List all available Lua & Luau detection rules and quality checks."""
    table = Table(
        title="🌙 DPX-Lua: Supported Pattern Rules & Quality Checks",
...
def info(
    pattern_name: Annotated[
        str,
        typer.Argument(
            help="Pattern type or name to query (e.g. 'metatable_prototype_oop', 'coroutine_cooperative_task').",
        ),
    ],
) -> None:
    """Display deep architectural details, rationale, and advice for a specific Lua/Luau pattern."""
    ...
    panel = Panel.fit(
        ...
        f"[bold white]Lua Target:[/bold white] {pdef.lua_version}\n\n"
        f"[bold white]Description:[/bold white]\n{pdef.description}\n\n"
        f"[bold white]Architecture Recommendation:[/bold white]\n{pdef.recommendation or 'Follow standard Lua metatable / Luau practices.'}",
        title=f"🔍 Lua Architecture Encyclopedia: {pdef.name}",
        border_style="cyan",
    )
    console.print(panel)
