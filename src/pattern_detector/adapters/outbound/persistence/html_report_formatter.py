"""Interactive HTML Observability HUD Formatter for Lua & Luau."""
...
CATEGORY_COLORS: dict[PatternCategory, dict[str, str]] = {
    PatternCategory.LUA_IDIOMATIC_METATABLE: {
        "text": "#00a2ff",
        "bg": "#00a2ff22",
        "border": "#00a2ff",
        "accent": "#54c4ff",
        "name": "Metatable & Idiomatic OOP",
    },
    PatternCategory.GAMEDEV_ROBLOX_NEOVIM: {
        "text": "#ff79c6",
        "bg": "#ff79c622",
        "border": "#ff79c6",
        "accent": "#ff92d0",
        "name": "GameDev & Engine Architecture",
    },
    PatternCategory.LUAJIT_FFI_SYSTEMS: {
        "text": "#50fa7b",
        "bg": "#50fa7b22",
        "border": "#50fa7b",
        "accent": "#73fa96",
        "name": "LuaJIT FFI & Systems",
    },
    ...
    PatternCategory.HAZARDS_MEMORY_LEAK: {
        "text": "#ff5555",
        "bg": "#ff555522",
        "border": "#ff5555",
        "accent": "#ff6e6e",
        "name": "Hazards & Memory Leaks",
    },
...
class HtmlReportFormatter(ReportFormatterPort):
    """Renders standalone, responsive HTML dashboard for Lua/Luau DetectionReport with AI Prompt Generator."""
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        ...
        for idx, det in enumerate(report.detections, 1):
            ...
            finding_prompt = (
                f"Refactor and optimize the following Lua / Luau architectural finding:\n"
                ...
                f"Please provide an idiomatic Lua 5.1-5.4 / Luau / LuaJIT implementation adhering to metatable prototype OOP, local scoping, coroutine concurrency, and zero global leaks."
            )
...
<head>
    ...
    <title>Pattern Scanner Report - DPX-Lua Architecture HUD - {html.escape(report.project_path or "Codebase")}</title>
    <style>
        :root {{
            --bg: #0a0d14;
            --card-bg: #141924;
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, monospace, sans-serif; }}
        ...
        .btn-ai-context {{
            background: linear-gradient(135deg, #00a2ff 0%, #ff79c6 100%);
            color: #ffffff;
            border: none;
            padding: 10px 18px;
            border-radius: 8px;
        ...
        .btn-ai-context:hover {{
            ...
            box-shadow: 0 6px 20px rgba(0, 162, 255, 0.5);
        }}
        .btn-view-prompt {{
        ...
        .btn-all {{ background: #1e2536; color: #f0f6fc; border-color: #3b4254; }}
        ...
        .search-bar:focus {{ border-color: var(--lua-blue); }}
        ...
        .pattern-card:hover {{ border-color: #00a2ff88; box-shadow: 0 4px 20px rgba(0,0,0,0.3); }}
        .card-header {{ background: #1c2333; padding: 12px 18px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px; border-bottom: 1px solid var(--border); }}
        ...
        code {{ background: #0a0d14; padding: 3px 6px; border-radius: 4px; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; color: #54c4ff; border: 1px solid #232b3b; }}
        ...
        .location-tag {{ color: #54c4ff; font-size: 11px; margin-left: 6px; }}
...
<body>
    <div class="container">
        <header>
            <div>
                <h1>🌙 DPX-Lua Architecture & GameDev Observability HUD</h1>
                <div class="subtitle">Metatable Prototype OOP, Coroutines, LuaJIT FFI, Roblox/Neovim Extensions & GoF 23 • Target: <code>{html.escape(report.project_path or "Target Repository")}</code></div>
            </div>
        ...
        <input type="text" id="searchInput" class="search-bar" placeholder="🔎 Instant search by pattern name, category, target class/function, or rule (e.g. metatable, coroutine, ffi, roblox, neovim, global_leak)...">
    ...
    <div id="promptModal" class="modal-overlay" onclick="closeModalOnOverlay(event)">
        <div class="modal-box">
            <div class="modal-header">
                <div class="modal-title">🤖 AI Systems Architect Context Prompt (Lua & Luau)</div>
                <button class="modal-close" onclick="closePromptModal()">&times;</button>
            </div>
            <div class="modal-body">
                <p style="font-size: 13px; color: #94a3b8; margin-bottom: 12px;">Structured XML/Markdown context for LLMs (Claude, GPT-4, Gemini) including metatable prototype OOP, coroutines, LuaJIT FFI, GameDev signals, and memory leak analysis.</p>
    ...
    <script>
        ...
        filterBtns.forEach(btn => {{
            btn.addEventListener('click', () => {{
                ...
                btn.style.outline = '2px solid #00a2ff';
                filterCards();
            }});
        }});
    </script>
