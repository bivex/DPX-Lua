"""JSON, Markdown, SARIF formatters & persistence repositories for Lua & Luau."""
...
class MarkdownReportFormatter(ReportFormatterPort):
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        lines = [
            f"# 🌙 DPX-Lua: Lua & Luau Architectural Pattern Report",
            f"",
        ...
        lines.extend([
            f"",
            f"## 🔍 Detailed Pattern Instances & Smells",
            f"",
        ])
...
class SarifReportFormatter(ReportFormatterPort):
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        ...
        for det in report.detections:
            ...
            level = "error" if det.pattern_category.value in ("hazards_memory_leak", "principle") else "note"
            ...
            "runs": [
                {
                    "tool": {
                        "driver": {
                            "name": "DPX-Lua",
                            "informationUri": "https://github.com/bivex/DPX-Lua",
                            "version": "0.1.0",
                        }
                    },
                    "results": results,
