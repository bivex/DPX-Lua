"""Token-Efficient LLM / AI Prompt Context Formatter for Lua & Luau."""
...
class LlmReportFormatter:
    ...
    def format_scan_report(self, report: DetectionReport) -> str:
        ...
        lines: list[str] = [
            '<codebase_architecture_analysis language="lua">',
        ...
        patterns = [d for d in report.detections if d.pattern_category not in (PatternCategory.PRINCIPLE, PatternCategory.HAZARDS_MEMORY_LEAK)]
        hazards = [d for d in report.detections if d.pattern_category in (PatternCategory.PRINCIPLE, PatternCategory.HAZARDS_MEMORY_LEAK)]

        if patterns:
            lines.append("    <lua_metatable_patterns_and_gamedev>")
            for d in patterns:
        ...
        if hazards:
            lines.append("    <lua_hazards_and_memory_leaks>")
            for v in hazards:
