"""Inbound driving ports for DPX-Lua."""
...
@dataclass
class ScanOptions:
    """Configuration options for a Lua/Luau scanning session."""

    min_confidence: float = 0.0
    enabled_patterns: list[str] = field(default_factory=list)
    file_extensions: list[str] = field(default_factory=lambda: [".lua", ".luau"])
...
class ScannerPort(Protocol):
    """Inbound port for scanning a target directory or Lua source file."""
